...
Component({
    ...
    methods: {
        ...
        addMaxWidthToStyle(htmlContent) {
            const maxWidth = '100%';
            // @ts-ignore
            htmlContent = htmlContent.replace(/(<img\s+[^>]*?style=)(["'])([^"']*)\2|(<img\s+(?!style=)[^>]*>)/gi, function (match, p1, p2, p3, p4) {
                if (p1) {
                    // 已有style属性
